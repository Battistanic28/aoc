package main

import (
	"strconv"
	"strings"
)

type Range struct {
	Start int64
	End   int64
}

func parseInputString(str string) []Range {
	str = strings.ReplaceAll(str, "\r\n", "\n")
	idRanges := strings.Split(strings.TrimSpace(str), ",")
	var s []Range

	for _, str := range idRanges {
		values := strings.Split(str, "-")
		start, _ := strconv.ParseInt(values[0], 10, 64)
		end, _ := strconv.ParseInt(values[1], 10, 64)

		r := Range{
			Start: start,
			End:   end,
		}

		s = append(s, r)
	}

	return s
}

var parsedInput = parseInputString(inputStr)

var inputStr = "9100-11052,895949-1034027,4408053-4520964,530773-628469,4677-6133,2204535-2244247,55-75,77-96,6855-8537,55102372-55256189,282-399,228723-269241,5874512-6044824,288158-371813,719-924,1-13,496-645,8989806846-8989985017,39376-48796,1581-1964,699387-735189,85832568-85919290,6758902779-6759025318,198-254,1357490-1400527,93895907-94024162,21-34,81399-109054,110780-153182,1452135-1601808,422024-470134,374195-402045,58702-79922,1002-1437,742477-817193,879818128-879948512,407-480,168586-222531,116-152,35-54"
